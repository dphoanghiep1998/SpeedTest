...
import com.example.speedtest.core.download.DownloadStream;
import com.example.speedtest.core.getIP.GetIP;
import com.example.speedtest.core.ping.PingStream;
import com.example.speedtest.core.serverSelector.TestPoint;
import com.example.speedtest.core.upload.UploadStream;
...
public abstract class SpeedtestWorker extends Thread{
    ...
    private void pingTest(){
        ...
        PingStream ps=new PingStream(backend.getServer(),backend.getPingURL(),config.getCount_ping(),config.getErrorHandlingMode(),config.getPing_connectTimeout(),config.getPing_soTimeout(),config.getPing_recvBuffer(),config.getPing_sendBuffer()) {
            ...
            @Override
            public void onError(String err) {
                Log.d("TAG", "onError: " + backend.getServer() + backend.getPingURL());
                abort();
                onCriticalFailure(err);
            }
